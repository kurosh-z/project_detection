...
def voc_eval(
    pred_bboxes, pred_labels, pred_scores, gt_bboxes, gt_labels, gt_difficults=None, iou_thresh=0.5, use_07_metric=True
):
    ...
    return eval_detection_voc(
        pred_bboxes, pred_labels, pred_scores, gt_bboxes, gt_labels, gt_difficults, iou_thresh, use_07_metric
    )
...
def eval_detection_voc(
    pred_bboxes, pred_labels, pred_scores, gt_bboxes, gt_labels, gt_difficults=None, iou_thresh=0.5, use_07_metric=False
):
...
def calc_detection_voc_prec_rec(
    pred_bboxes, pred_labels, pred_scores, gt_bboxes, gt_labels, gt_difficults=None, iou_thresh=0.5
):
...
def _prepare_data(path):
    ...
    _path = os.path.join(path, "k2dTrain.txt")
    names_txt = open(_path, "r")
    filenames = names_txt.readlines()
    trainX = np.zeros((len(filenames), 185, 306, 3))
    trainY = []
    for idx, nameX in enumerate(filenames):
        nameX = nameX.rstrip()
        exploded = nameX.split("_")
        nameY = exploded[0] + "_" + exploded[1].replace("X", "Y") + "_" + exploded[2]
        X = np.load(os.path.join(path, "train/" + nameX), allow_pickle=True)
        trainX[idx, ...] = X
        Y = np.load(os.path.join(path, "train/" + nameY), allow_pickle=True)
        classes = Y[:, 0]
        boxes = Y[:, 1:6]
        boxes = [np.array(b) for b in boxes]
        trainY.append({"classes": classes, "boxes": boxes})
...
if __name__ == "__main__":
    ...
    store_path = f"data/"  # adapt to your file location
    x_train, y_train = _prepare_data(store_path)
    # read prediction
    ...
    pred_test = np.load(os.path.join(store_path, "train_predictions.npy"), allow_pickle=True)
    ...
    # pred_scores = [torch.Tensor(x["scores"]) if x["scores"] is not None else torch.Tensor([0]) for x in pred_test] # List with n_test elements [n_boxes]

    pred_boxes = []
    pred_classes = []
    pred_scores = []
    for x in pred_test:
        pred_boxes.append(torch.Tensor(x["boxes"]))
        pred_classes.append(torch.Tensor(x["classes"]))
        pred_scores.append(torch.Tensor(x["scores"]))

    # read gt
    # gt = np.load(os.path.join(store_path, "y_train.npy"), allow_pickle=True)
    gt = y_train
    gt_boxes = [torch.Tensor(x["boxes"]) for x in gt]  # List with n_test elements [n_boxes, 4]
    gt_labels = [torch.Tensor(x["classes"]) for x in gt]  # List with n_test elements [n_boxes]
